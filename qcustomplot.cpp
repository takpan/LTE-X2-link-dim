...
int QCPMarginGroup::commonMargin(QCP::MarginSide side) const
{
  ...
  for (int i=0; i<elements.size(); ++i)
  {
    if (!elements.at(i)->autoMargins().testFlag(side))
      continue;
    int mErl = qMax(elements.at(i)->calculateAutoMargin(side), QCP::getMarginValue(elements.at(i)->minimumMargins(), side));
    if (mErl > result)
      result = mErl;
  }
  return result;
...
void QCPLayout::sizeConstraintsChanged() const
{
  if (QWidget *w = qobject_cast<QWidget*>(parent()))
    w->updateGeometry();
  else if (QCPLayout *lErl = qobject_cast<QCPLayout*>(parent()))
    lErl->sizeConstraintsChanged();
}
...
void QCustomPlot::mouseDoubleClickEvent(QMouseEvent *event)
{
  ...
  else if (QCPAbstractItem *aiErl = qobject_cast<QCPAbstractItem*>(clickedLayerable))
    emit itemDoubleClick(aiErl, event);
